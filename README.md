# chatai-wide-screen-enhancer

DeepSeek 网页端宽屏 + 表格显示优化 + 自动折叠深度思考

## 功能

- 📐 **宽屏模式** — 将聊天内容撑满屏幕，告别窄版
- 📊 **表格加宽** — 表格自适应宽度，斑马纹 + 暗色适配
- 🧠 **自动折叠深度思考** — DeepSeek 思考过程自动收起，只看最终回复

## 安装

需要浏览器安装 [Tampermonkey](https://www.tampermonkey.net/) 或 [Violentmonkey](https://violentmonkey.github.io/)，然后打开下方的脚本链接安装：

[👉 安装脚本](https://github.com/NBSOD/chatai-wide-screen-enhancer/raw/main/chatai-wide-screen-enhancer.user.js)

## 变更

| 版本 | 内容 |
|------|------|
| 1.0.15 | 移除冲突的 wideMode CSS，extraCSS 始终生效即可达到最佳宽屏效果，输入框不受影响 |
| 1.0.14 | 修复宽屏导致右侧内容被截断：将宽屏 CSS 限定在 `.ds-virtual-list-items` 对话区域内，不再影响输入框/侧边栏 |
| 1.0.13 | 输入框保持原始窄度；设置移至脚本管理器菜单（GM_registerMenuCommand），移除网页齿轮图标 |
| 1.0.12 | 自适应宽屏：使用 `min(98%, 1600px)` 根据分辨率动态缩放，超宽屏自动封顶 |
| 1.0.11 | 真正宽屏：消除两侧大量空白，覆盖 ds-virtual-list-items 内边距和 CSS 变量 |
| 1.0.10 | 修复 v1.0.9 完全不会折叠思考过程的问题：放宽文本匹配条件 |
| 1.0.9 | 修复 hideThinkingEarly 误伤输入框切换按钮导致对话跳信息的问题 |
| 1.0.8 | 修复滚动历史记录时自动下滑问题；移除 Gemini 支持，仅对 DeepSeek 生效 |
| 1.0.7 | 重写自动折叠逻辑，不再依赖"已思考"文本，直接从 DOM 结构判断 |
| 1.0.4 | 修复折叠逻辑：根据实际 DOM 结构重写，定位"已思考"文本 + 点击 chevron 图标 |
| 1.0.3 | 新增自动折叠深度思考功能 |
| 1.0.2 | 移除代码美化（Gemini 兼容问题） |
| 1.0.1 | 修复 Gemini 代码块颜色问题 |
| 1.0.0 | 初始版本 |